...
**How:** wrap the query in `readAllPages` (`src/lib/pagedRead.ts`), with a total order (`date`, then `id`) and `.range(from, to)`; it loops until a page comes back short. Used by `useTransactions`, `useBudgets` and `useSavingsGoals`; `useOverspending` and `useImportDuplicates` inline the same loop.
