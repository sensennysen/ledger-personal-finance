...
function MacWindow({
  ...
  sidebar, children, bare = false, dark = false,
}) {
  // bare: titlebar chrome only (traffic lights + centered title) — the app
  // inside draws its own navigation and fills the whole window body.
  if (bare) {
    return (
      <div data-om-starter="macos-window" style={{
        width, height, borderRadius: 12, overflow: 'hidden',
        background: dark ? '#15130B' : '#fff',
        boxShadow: '0 0 0 1px rgba(0,0,0,0.23), 0 16px 48px rgba(0,0,0,0.35)',
        display: 'flex', flexDirection: 'column', position: 'relative',
        fontFamily: MAC_FONT, boxSizing: 'border-box',
      }}>
        <div style={{
          height: 38, flexShrink: 0, display: 'flex', alignItems: 'center',
          padding: '0 14px', gap: 12,
          background: dark ? '#1D1B13' : '#F2EDE4',
          borderBottom: dark ? '1px solid #2C2921' : '1px solid #DDD5C4',
        }}>
          <MacTrafficLights />
          <div style={{
            flex: 1, textAlign: 'center', paddingRight: 64,
            fontFamily: MAC_FONT, fontSize: 13, fontWeight: 600,
            color: dark ? 'rgba(233,226,212,0.75)' : 'rgba(0,0,0,0.6)',
          }}>{title}</div>
        </div>
